...
namespace ModSink.Application.Hashing {
  public class StreamBreaker {
    ...
    /// <summary>
    ///     Subdivides a stream using a Rabin-Karp rolling hash to find sentinal
    ///     locations
    /// </summary>
    /// <param name="stream">The stream to read</param>
    /// <param name="length">The length to read</param>
    /// <param name="hasher">A hash algorithm to create a strong hash over the
    /// segment</param> <remarks>
    ...
    ///     So...we keep track of length and don't arbitrarily read 'til we get
    ///     zero Also, overflows occur when getting maxSeed and when calculating
    ///     the hash. These overflows are expected and not significant to the
    ///     computation.
    /// </remarks>
    public IEnumerable<Segment>GetSegments(Stream stream, long length) {
      ...
      var circle =
          new byte[width]; //--> the circular queue: input to the hash functions
      var hash = 0L;       //--> our rolling hash
      var circleIndex = 0; //--> index into circular queue
      var last = 0L;       //--> last place we started a new segment
      var pos =
          0L; //--> the position we're at in the range of stream we're reading
      ...
      while (true) {
        ...
        var bytesRead =
            stream.Read(buffer, 0, (int) Math.Min(bufferSize, length - pos));
        for (var i = 0; i < bytesRead; i++) {
          ...
          {
            //--> apply the strong hash to the remainder of the bytes in the
            //circular queue...
            hasher.TransformFinalBlock(circle, 0,
                                       circleIndex == 0 ? width : circleIndex);

            //--> return the results to the caller...
            yield return new Segment(last, pos - last,
                                     _hashFunction.CreateHash(hasher.Hash));
            last = pos;
            ...
            if (circleIndex == 0)
              hasher.TransformBlock(circle, 0, width, circle, 0);
          }
        }
    ...
    public struct Segment {
      ...
      public Chunk ToChunk() {
        return new Chunk{Position = Offset,
                         Signature = new Signature(Hash, Length)};
      }
    }
  }
}
