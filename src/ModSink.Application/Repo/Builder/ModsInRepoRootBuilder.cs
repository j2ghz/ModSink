...
namespace ModSink.Application.Repo.Builder {
  public class ModsInRepoRootBuilder
      : IRepoBuilder<ModsInRepoRootBuilderConfig> {
    ...
    public Task<RepoWithFileChunks>Build(IDirectoryInfo root,
                                         CancellationToken token) {
      return Build(root, null, token);
    }

    public async Task<RepoWithFileChunks>
    Build(IDirectoryInfo root, ModsInRepoRootBuilderConfig? config,
          CancellationToken token) {
      if (config == null)
      ...
      foreach(var modName in allModNames) {
        ...
        builtMods.Add(
            new Mod{Files = modFiles.Select(t => t.Result.File).ToList(),
                    Name = modName});
      }

      var modpacks = config.Modpacks.Select(
          modpack =>
              new Modpack{Name = modpack.Name,
                          Mods = modpack.Mods
                                     .Select(modName => builtMods.First(
                                                 mod => mod.Name == modName))
                                     .ToList()});

      var repo =
          new Domain.Entities.Repo.Repo(config.Name, modpacks.ToList(), null);
      return new RepoWithFileChunks(repo, repoChunks);
    }
  }

  public class ModsInRepoRootBuilderConfig {
    public ModsInRepoRootBuilderConfig(string name,
