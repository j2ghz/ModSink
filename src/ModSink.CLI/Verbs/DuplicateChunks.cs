  ...
  public class DuplicateChunks : BaseVerb<DuplicateChunks.Options> {
    ...
    public override int Run(Options options) {
      ...
      if (fileSystem.File.Exists(options.Path))
        ...
        files.AddRange(
            GetFiles(fileSystem.DirectoryInfo.FromDirectoryName(options.Path)));
      ...
      var sizes =
          new Dictionary<string, long>{{"JSON (Newtonsoft)", 0L},
                                       {"JSON (Newtonsoft) (Brotli)", 0L},
                                       {"MessagePackSerializer", 0L},
      ...
      foreach(var file in files) {
        ...
        var bytes =
            Encoding.Default.GetBytes(JsonConvert.SerializeObject(metadata));
        sizes["JSON (Newtonsoft)"] += bytes.LongLength;
        var compressedStream = new MemoryStream();
        using var b =
            new BrotliStream(compressedStream, CompressionLevel.Fastest);
        b.Write(bytes);
        sizes["JSON (Newtonsoft) (Brotli)"] += compressedStream.Length;
        sizes["MessagePackSerializer"] +=
            MessagePackSerializer
                .Serialize(metadata, ContractlessStandardResolver.Instance)
                .Length;
        sizes["MessagePackSerializer (LZ4)"] +=
      ...
      sw.Stop();
      Console.WriteLine(
          $"Processed {size.Bytes().Humanize(f)} of files in {sw.Elapsed}");

      foreach(var (key, value) in sizes)
          Console.WriteLine($"{key}\t{value.Bytes().Humanize(f)}");
      return 0;
    }

    public class ByteArrayComparer
        : IEqualityComparer<byte[]>{public bool Equals(byte[] a, byte[] b){
...
public class Metadata {
  ...
  public StreamBreaker.Segment[] Segments {
    get;
    set;
  }
}
