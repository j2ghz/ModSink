    ...
    public class HttpClientDownloader : IDownloader
    {
        ...
        public IObservable<DownloadProgress> Download(Uri source, Stream destination)
        {
            return Observable.Create<DownloadProgress>(async (observer, cancel) =>
            {
                ...
                var response = await client.GetAsync(source, HttpCompletionOption.ResponseHeadersRead, cancel);
                ...
                observer.OnNext(new DownloadProgress(0, length, DownloadProgress.DownloadState.ReadingResponse));
