...
namespace ModSink.Domain.Tests.Entities.Repo {
  public class RelativeUriTests {
    ...
    public void
    ConstructFailsFromInvalidString(string url) {
      try {
        var uri = new RelativeUri(url);
        uri.ToString().Should().Be(
            "", "because AbosluteUris shouldn't make relative ones");
    ...
    [InlineData("a/b.txt")]
    public void
    ConstructsFromValidString(string url) {
      var uri = new RelativeUri(url);
    ...
    public void
    FromAbsolute(string root, string target, string result) {
    ...
    [Fact]
    public void
    HackWhenCreatingFromDirectoryInfo() {
      var fileSystem = new MockFileSystem(new Dictionary<string, MockFileData>{
          {@"/d/a.txt", new MockFileData("a")}});
      var d = fileSystem.DirectoryInfo.FromDirectoryName("d");
      d.FullName.Should().NotEndWith(
          "/",
          "because paths to directories should end with /, but for some reason they don't");

      var wrongUri = RelativeUri.FromAbsolute(
          new Uri(d.FullName), new Uri(d.GetFiles().Single().FullName));
      wrongUri.ToString().Should().Be("d/a.txt");

      var correctUri = RelativeUri.FromAbsolute(
          new Uri(d.FullName + "/"), new Uri(d.GetFiles().Single().FullName));
