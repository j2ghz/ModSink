...
namespace ModSink.Domain.Entities {
  public readonly struct DownloadProgress {
    ...
    /// <param name="downloaded">Size of the downloaded part, zero if not
    /// specified</param>
    public DownloadProgress(TransferState state, ByteSize size = default,
                            ByteSize downloaded = default) {
      Size = size;
    ...
    public override string ToString() {
      return $"{State.Humanize(LetterCasing.Sentence)} {Downloaded.Humanize(" G03
             ")} out of {Size.Humanize(" G03 ")}";
    }
