...
#region Generated
    ...
    public override int GetHashCode() {
      unchecked {
        return ((HashId != null ? HashId.GetHashCode() : 0) * 397) ^
    ...
    public byte[] RawForHashing() {
      var result = HashId.Aggregate(new byte[0].AsEnumerable(),
                                    (current, t) => current.Append((byte) t));
      result = Value.Aggregate(result, (current, b) => current.Append(b));
      return result.ToArray();
    }

    public override string ToString() =>
        $"Hash(Id='{HashId}',Value='{string.Join(",
                           ", Value)}')";
  }
}
