...
namespace ModSink.Infrastructure.Hashing {
  public class HashingService : IHashingService, IDisposable {
    ...
    public HashingService(IHashFunction hashFunction, IOptions<Options>options,
                          IFileOpener fileOpener, StreamBreaker streamBreaker) {
    ...
    public IEnumerable<Task<(RelativePathFile File, List<Chunk>Chunks)>>
    GetFileHashes(IDirectoryInfo directory, CancellationToken token) {
      ...
      foreach(var file in GetFiles(directory)) {
        ...
        var hash = RunASyncWaitSemaphore(async () => {
          ...
          return (
              new RelativePathFile{Signature = fileSignature,
    ...
    public async Task<(Signature fileSignature, List<Chunk>fileChunks)>
    GetFileHash(IFileInfo file, CancellationToken cancel) {
      using var stream = _fileOpener.OpenRead(file);
      var chunks = _streamBreaker.GetSegments(stream, file.Length).ToList();
      var chunkHashesStream = new MemoryStream(CombineByteArrays(
          chunks.Select(c => c.Hash.RawForHashing()).ToArray()));
      var fileHash =
          await _hashFunction.ComputeHashAsync(chunkHashesStream, cancel);
      var fileSig = new Signature(fileHash, file.Length);
      var fileChunks =
          chunks
              .Select(c =>
                          new Chunk{Signature = new Signature(c.Hash, c.Length),
                                    Position = c.Offset})
              .ToList();
    ...
    public async Task<Hash>GetFileHash(Stream stream,
    ...
    RunASyncWaitSemaphore<T>(Func<Task<T>>action, SemaphoreSlim semaphoreSlim,
